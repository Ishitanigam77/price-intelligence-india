...
Multi-stage builds, lockfile/pyproject installs, no secrets in layers. `CLERK_SECRET_KEY` is runtime-only. `NEXT_PUBLIC_*` are build-args because Next.js inlines them for the browser (they are not server secrets). Runtime stages run `apt-get upgrade` / `apk upgrade` so Debian/Alpine security patches (for example OpenSSL) land in the image Trivy scans. The frontend runtime image also removes unused `npm`/`npx`/`corepack` (Next.js standalone only needs `node`). Do not ignore HIGH/CRITICAL image CVEs that already have a fix.
