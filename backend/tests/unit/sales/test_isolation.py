"""Sale-event package stays independent of FastAPI, named retailers, and ML."""

import ast
from pathlib import Path

BACKEND_ROOT = Path(__file__).resolve().parents[3]
SALES_ROOT = BACKEND_ROOT / "app/sales"
...
def test_sales_package_does_not_import_fastapi_named_adapters_or_ml() -> None:
    offenders: list[str] = []
    for path in _python_files(SALES_ROOT):
...
def test_sales_contains_no_retailer_identity_branches() -> None:
    offenders: list[str] = []
    for path in _python_files(SALES_ROOT):
